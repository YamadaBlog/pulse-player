...
// Local drag-position ref. Renamed from `position` to `dragPos` so it
// doesn't shadow the `position` prop (script-setup compilation behaves
// fine, but the duplicate name was a real readability tax).
...
function onPointerMove(e: PointerEvent) {
  ...
  // (see onPointerUp).
  dragPos.value = { x: dragStartPos.x + dx, y: dragStartPos.y + dy }
}
...
function isFullyVisible(p: { x: number; y: number }): boolean {
  ...
  return (
    left >= 0 && top >= 0 && left + size <= window.innerWidth && top + size <= window.innerHeight
  )
}
...
function onPointerUp() {
  ...
  if (!isFullyVisible(dragPos.value)) {
    dragPos.value = { x: 0, y: 0 }
    persistPosition()
    return
  }
...
function persistPosition() {
  ...
  try {
    localStorage.setItem(props.persistKey, JSON.stringify(dragPos.value))
  } catch {
...
function restorePosition() {
  ...
  try {
    ...
    dragPos.value = isFullyVisible(parsed) ? parsed : { x: 0, y: 0 }
  } catch {
...
function onWindowResize() {
  if (dragPos.value.x === 0 && dragPos.value.y === 0) return
  if (!isFullyVisible(dragPos.value)) {
    dragPos.value = { x: 0, y: 0 }
    persistPosition()
  }
...
const fabStyle = computed(() => {
  ...
  if (dragPos.value.x !== 0 || dragPos.value.y !== 0) {
    base.transform = `translate(${dragPos.value.x}px, ${dragPos.value.y}px)`
    base.transition = isDragging.value
      ? 'none'
...
.fab--pulso .fab__btn:hover {
  animation: none;
  transform: scale(1.06);
}
.fab--pulso.fab--dragging .fab__btn {
  animation: none;
}
...
.fab[data-variant='solid'] .fab__btn {
  background: var(--pulse-bg, #14141a);
}
.fab[data-variant='dark'] .fab__btn {
  background: #0a0a0f;
}
...
.fab[data-variant='light'] .fab__icon {
  color: #14141a;
  filter: none;
}
...
.fab[data-variant='light'] .fab__ring-track {
...
.menu-pop-enter-from .fab__menu-btn--next {
  transform: translate(20px, 10px) scale(0.5);
}
.menu-pop-enter-from .fab__menu-btn--close {
  transform: translate(-20px, 10px) scale(0.5);
}
.menu-pop-leave-to {
  opacity: 0;
