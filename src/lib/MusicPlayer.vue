...
function computeScale(width: number): number {
  ...
  if (width >= 280) {
    ...
    s = 0.45 + ((width - 170) / 110) * 0.4
  }
  return Math.max(0.45, Math.min(1.8, Number(s.toFixed(3))))
}

const scale = computed(() => (typeof props.size === 'number' ? props.size : autoScale.value))
...
function onResizePointerMove(e: PointerEvent) {
  ...
  const clamped = Math.max(props.minWidth, props.maxWidth ? Math.min(props.maxWidth, next) : next)
...
function onResizePointerUp(e: PointerEvent) {
  ...
  try {
    ...
    /* pointer capture already released (browser quirk) — safe to ignore */
  }
}
...
.mp[data-variant='solid'] {
  background: var(--pulse-bg, #14141a);
}
.mp[data-variant='dark'] {
  background: #0a0a0f;
}
...
.mp[data-fab='true'] .mp__art-img {
  border-radius: 50%;
}
.mp[data-fab='true'] .mp__art-hover {
  opacity: 0 !important;
  pointer-events: none;
}
...
.mp[data-fab='true'] .mp__resize svg {
  width: 11px;
  height: 11px;
}
...
.mp[data-narrow='true'] .mp__now-label {
  display: none;
}
.mp[data-narrow='true'] .mp__icons {
  gap: max(4px, calc(6px * var(--pulse-scale)));
}
...
.mp[data-compact='true'] .mp__spacer {
  display: none;
}
...
.mp[data-compact='true'] .mp__meta {
  margin-bottom: 0;
}
.mp[data-compact='true'] .mp__controls {
  gap: calc(var(--pulse-pad) * 0.4);
}
...
.mp__art:hover .mp__art-hover {
  opacity: 1;
}
.mp__art-hover svg {
  width: calc(var(--pulse-art) * 0.28);
  height: calc(var(--pulse-art) * 0.28);
}
...
.mp[data-variant='light'] .mp__now-label {
  color: rgba(20, 20, 26, 0.55);
}
.mp[data-variant='vinyl'] .mp__now-label {
...
.mp__eq {
  display: flex;
  align-items: flex-end;
  gap: var(--pulse-eq-gap);
  height: var(--pulse-eq-h);
}
...
.mp__icon-link:not(.mp__icon-link--decorative) {
  cursor: pointer;
}
.mp__icon-link--decorative {
  cursor: default;
  pointer-events: none;
}
.mp__icon-link:hover {
...
.mp[data-variant='light'] .mp__icon-link {
  color: rgba(20, 20, 26, 0.5);
}
.mp[data-variant='light'] .mp__icon-link:hover {
  color: #14141a;
}
.mp[data-variant='vinyl'] .mp__icon-link {
  color: rgba(245, 240, 232, 0.4);
}
.mp[data-variant='vinyl'] .mp__icon-link:hover {
  color: #f5f0e8;
}
.mp__icon-link--spotify {
  color: #1db954;
  opacity: 0.92;
}
.mp__icon-link--spotify:hover {
  color: #1db954;
  opacity: 1;
}
...
.mp__spacer {
  flex: 1;
  min-height: var(--pulse-spacer);
}
.mp__meta {
  margin-bottom: calc(var(--pulse-pad) / 3);
  min-width: 0;
}
...
.mp[data-variant='light'] .mp__title {
  text-shadow: none;
  color: #14141a;
}
.mp[data-variant='transparent'] .mp__title {
  text-shadow: 0 1px 6px rgba(0, 0, 0, 0.4);
}
.mp[data-variant='vinyl'] .mp__title {
  color: #f5f0e8;
...
.mp__btn svg {
  width: var(--pulse-btn-icon);
  height: var(--pulse-btn-icon);
}
.mp__btn:hover {
  color: var(--pulse-text, rgba(255, 255, 255, 0.95));
  transform: scale(1.1);
...
.mp[data-variant='light'] .mp__btn:hover {
  color: #14141a;
}
.mp[data-variant='vinyl'] .mp__btn {
  color: rgba(245, 240, 232, 0.6);
}
.mp[data-variant='vinyl'] .mp__btn:hover {
  color: #f5f0e8;
}
...
.mp[data-variant='light'] .mp__bar {
  background: rgba(0, 0, 0, 0.07);
}
.mp__bar:hover {
  height: var(--pulse-bar-h-hov);
}
.mp__fill {
  height: 100%;
...
.mp[data-variant='light'] .mp__fill {
  background: rgba(20, 20, 26, 0.35);
}
.mp__bar:hover .mp__fill {
...
.mp__resize--active {
  color: var(--pulse-accent, #3dbda7);
}
.mp[data-variant='light'] .mp__resize {
  color: rgba(20, 20, 26, 0.4);
}
.mp[data-variant='light'] .mp__resize:hover {
  color: rgba(20, 20, 26, 0.92);
}
