...
`@pulse-music/vue` now exposes the same surface as the historical `pulse-player` package:
...
```ts
// Install (once v3.0.0 stable ships): npm install @pulse-music/vue
...
| Aspect | `@pulse-music/vue` (v2.3.4 ref) | `@pulse-music/react` / `svelte` / `angular` |
| --- | --- | --- |
| Chrome parity | **100 %** | ~85 % (chrome being completed) |
| Backed by | Vue Pinia store + Vue SFCs | `@pulse-music/web-component` Lit Custom Elements |
...
If you want the **full premium chrome today**, use `@pulse-music/vue`. If you're building in another framework, use the corresponding wrapper.
